...
//       <h1 className="text-xl font-bold text-green-500">🎉 Order Placed Successfully!</h1>
//       {order ? (
...
//           <p className="text-blue-500">📢 We will notify you when your order is shipped!</p>
//         </div>
//       ) : (
...
const OrderConfirmation = () => {
    ...
    try {
      ...
      if (orderSnap.exists()) {
        ...
        const productPromises = orderData.products.map(async (product) => {
          const productRef = doc(db, 'products', product.productId);  // Correctly reference product.productId
          const productSnap = await getDoc(productRef);
          console.log({ id: product.productId, ...productSnap.data() });
          return productSnap.exists() ? { id: product.productId, ...productSnap.data() } : null;
        });
  ...
  if (!order) return <div className="text-center">
      ...
      <div className="p-4">
        <div className="flex justify-between mt-4 text-sm">
          <div>
            ...
            <p>Total: <span className="font-semibold">₹{order?.grandTotal}</span></p>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-4 max-h-96 overflow-y-auto">
            {loading ? (
              Array(2).fill(0).map((_, idx) => <SkeletonLoader key={idx} />)
            ) : (
              products?.map((item) => {
                const productInOrder = order?.products?.find(p => p.productId === item.id);
                  ...
                  <div key={item.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-green-600">{item?.discountPercentage || 0}% off</p>
        ...
        <div className="mt-6 bg-blue-50 p-4 rounded-lg flex items-center gap-2">
          ...
          <p className="text-blue-600">Expected delivery: {order.estimatedDeliveryDate || 'To be determined'}</p>
        </div>

        <div className="flex justify-between mt-6">
          <Link to="/dashboard/orders" className="px-4 py-2 border rounded hover:bg-gray-50">
            View Orders
          </Link>
          <button className="px-4 py-2 border rounded hover:bg-gray-50">
            Need Help?
          </button>
          <Link to="/products" className="px-4 py-2 bg-black text-white rounded hover:bg-gray-800">
            Continue Shopping
          </Link>
        </div>
      </div>
