...
const TableCommandHeader = () => {
   ...
   * Validates nested object structure recursively
   */
  const validateNestedObjectStructure = (
    referenceObj,
    currentObj,
    propertyPath,
    objectIndex
    ...
    if (missingKeys.length > 0) {
      return {
        ...
        error: `Nested object "${propertyPath}" in object at position ${objectIndex} is missing properties: ${missingKeys.join(
          ", "
        )}.\n\nExpected nested structure: ${JSON.stringify(
          referenceObj,
          null,
          2
        )}\nActual nested structure: ${JSON.stringify(
          currentObj,
          null,
          2
        )}\n\nNested objects must have consistent structure.`,
  ...
  const validateStructuralConsistency = (data) => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (missingKeys.length > 0) {
        return {
          isValid: false,
          error: `Object at position ${
            i + 1
          } is missing required properties: ${missingKeys.join(
            ", "
          )}.\n\n📋 Expected properties: ${referenceKeys.join(
            ", "
          )}\n📋 Actual properties: ${currentKeys.join(
      ...
      if (extraKeys.length > 0) {
        return {
          ...
          error: `Object at position ${
            i + 1
          } has unexpected properties: ${extraKeys.join(
            ", "
          )}.\n\n📋 Expected properties: ${referenceKeys.join(
            ", "
          )}\n📋 Actual properties: ${currentKeys.join(
      ...
      for (const key of referenceKeys) {
        ...
        if (referenceType !== currentType) {
          return {
            isValid: false,
            error: `Property "${key}" has inconsistent data type in object at position ${
              i + 1
            }.\n\n📊 Expected type: ${referenceType}\n📊 Actual type: ${currentType}\n\n🔍 Reference value: ${JSON.stringify(
              referenceObject[key]
            )}\n❌ Current value: ${JSON.stringify(
  ...
  const validateArrayOfObjects = (data) => {
    ...
    if (data.length === 0) {
      return {
        isValid: false,
        error:
          "The array is empty. Please provide at least one object in the array.",
      };
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (item === null) {
        return {
          isValid: false,
          error: `Item at position ${
            i + 1
          } is null. Each array item must be a valid object.`,
        };
      }
      ...
      if (Array.isArray(item)) {
        return {
          ...
          error: `Item at position ${
            i + 1
          } is an array. Each array item must be an object, not a nested array.`,
        };
      }
      ...
      if (typeof item !== "object") {
        return {
          ...
          error: `Item at position ${
            i + 1
          } is of type "${typeof item}". Each array item must be an object with key-value pairs.`,
        };
      }
      ...
      if (Object.keys(item).length === 0) {
        return {
          ...
          error: `Item at position ${
            i + 1
          } is an empty object. Each object should have at least one property.`,
        };
      }
    }
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          message: error.message,
          suggestion:
            "Check JSON syntax for missing commas, quotes, or other formatting issues",
        },
      };
    }
  };
    ...
    try {
        ...
        try {
          ...
          if (Array.isArray(parsed) && parsed.length === 1) {
            return {
              ...
              wasWrapped: true,
              message:
                "Wrapped single object (with nested structure) in array brackets",
            };
          }
        } catch (e) {
          // If wrapping fails, the JSON itself is malformed
      ...
      if (!trimmed.startsWith("[") && !trimmed.endsWith("]")) {
        ...
        return {
          ...
          wasWrapped: true,
          message:
            "Attempted to wrap in array brackets (may need manual fixing)",
        };
      }
  ...
  const toggleDataMode = (mode) => {
    if (mode !== dataMode) {
      ...
      if (mode === "url") {
        ...
        // Automatically open JSON popup if no data exists
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !urlContainerRef.current.contains(event.target)
      ) {
        handleUrlClose();
      }
    };
      ...
      <div className="flex items-center justify-between space-x-5 bg-white p-2 my-1 z-10">
        ...
        <div className="flex gap-x-2 items-center">
          ...
          <div className="text-xl font-semibold mr-3">
            ...
            <Hammer
              size={40}
              className="inline-block p-1 rounded-full border-black border-2 mr-1"
            />
            </Link>
          ...
          <div className="flex items-center gap-x-3">
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-1">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer rounded-md text-sm font-medium transition-all ${
                  dataMode === "url"
                    ? "bg-white text-blue-600 shadow-sm ring-2 ring-blue-400"
              ...
              <button
                ...
                className={`flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                  dataMode === "json"
                    ? "bg-white text-purple-600 shadow-sm ring-2 ring-purple-400"
                    : "text-gray-600 hover:text-gray-800 hover:bg-gray-200"
                }`}
              >
                <Code size={16} />
            ...
            {dataMode === "url" && (
              <div
                ref={urlContainerRef}
                className={`
      relative rounded-full h-[2.5rem] flex items-center
      ${
        isUrlExpanded
          ? "w-[21rem] border-2 border-blue-500 bg-blue-50 shadow-md"
          : "w-[2.5rem] border-2 border-blue-400 bg-blue-50 hover:border-blue-500"
      }
      transition-all duration-300 ease-in-out overflow-hidden cursor-pointer
      ${dataMode === "url" ? "ring-2 ring-blue-200 ring-offset-1" : ""}
      animate-pulse-once
    `}
                onClick={handleUrlContainerClick}
              >
      ...
      `}
                ...
                {isUrlExpanded && (
                  <button
                    ...
                    className="absolute right-2 top-1/2 transform -translate-y-1/2 w-6 h-6 flex items-center justify-center rounded-full hover:bg-blue-200 transition-colors"
                  >
            ...
            {dataMode === "json" && (
              <button
                onClick={handleJsonPopupOpen}
                className="flex items-center gap-2 px-4 py-2 border-2 rounded-lg transition-colors relative"
                style={{
                  backgroundColor: rawJsonData?.length
                    ? "rgb(243 232 255)"
                    : "rgb(245 243 255)",
                  borderColor: rawJsonData?.length
                    ? "rgb(168 85 247)"
                    : "rgb(139 92 246)",
                  color: rawJsonData?.length
                    ? "rgb(126 34 206)"
                    : "rgb(107 33 168)",
                }}
              >
                ...
                <span className="text-sm font-medium">
                  {rawJsonData?.length
                    ? `${rawJsonData.length} Items Loaded`
                    : "Add JSON Data"}
                </span>
                {rawJsonData?.length && (
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col">
            ...
            <div className="px-6 py-4 bg-blue-50 border-b border-blue-100 flex-shrink-0">
              <p className="text-sm text-blue-800 flex items-center gap-2">
                <AlertCircle size={16} className="text-blue-600" />
                Enter JSON objects. If you paste objects without array brackets,
                we'll automatically wrap them for you.
              </p>
            </div>
            ...
            <div className="flex-1 flex flex-col p-6 min-h-0">
              ...
              {autoWrapMessage && (
                <div className="mb-2">
                  <div
                    className={`text-xs px-3 py-2 rounded-md ${
                      autoWrapMessage.includes("successful") ||
                      autoWrapMessage.includes("Wrapped")
                        ? "bg-green-100 text-green-700 border border-green-200"
                        : "bg-blue-100 text-blue-700 border border-blue-200"
                    }`}
                  >
              ...
              <div className="flex-1 flex gap-4 min-h-0">
                ...
                <div
                  className={`${
                    jsonError ? "flex-1" : "w-full"
                  } border border-gray-300 rounded-lg overflow-hidden bg-white shadow-inner`}
                >
                  <div className="flex h-full">
                    {/* Line Numbers Column */}
                    <div
                          ...
                          <div className="bg-white p-3 rounded-md border border-red-200">
                            ...
                            <ul className="text-xs text-red-700 space-y-1">
                              ...
                              <li className="flex items-start gap-2">
                                ...
                                <span>
                                  Try the Auto-Wrap feature for objects
                                </span>
                              </li>
                            </ul>
                          </div>
